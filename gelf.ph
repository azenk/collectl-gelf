...
sub gelfInit
{
  ...
  error('--showcolheader not supported by gelf')    if $showColFlag;
  ...
  foreach my $option (@_)
  {
    ...
    error("invalid gelf option '$name'")    if $name!~/^[dgGhis]?$|^align|^co$|^ttl$|^min$|^max$|^avg$|^tot$/;
  ...
  error("only 1 of 'g' or 'G' with 'gelf'")                            if $gexGFlag>2;
  error("gelf does not support standard collectl socket I/O via -A")   if $sockFlag;
  ...
  error("gelf subsys options '$gexSubsys' not a proper subset of '$subsys'")
  ...
  error("gelf interval of '$gexInterval' is not a multiple of '$gexColInt' seconds")
	if $gexColInt*$gexSendCount != $gexInterval;

  $gexFlags=$gexMinFlag+$gexMaxFlag+$gexAvgFlag+$gexTotFlag;
  error("only 1 of 'min', 'max', 'avg' or 'tot' with 'gelf'")    if $gexFlags>1;
  error("'min', 'max', 'avg' & 'tot' require gelf 'i' that is > collectl's -i")
  ...
  # Since gelf DOES write over a socket but does not use -A, make sure the default
...
sub gelf
{
  ...
  # if any imported data, it may want to include gelf output.  However this means getting a list of
...
sub openSocket
{
  ...
  $gexPaddr = pack_sockaddr_in($port,$iaddr) or logmsg('F', "Couldn't convert address for '$host'");
  my $proto = getprotobyname('tcp')     or logmsg('F', "Couldn't getprotbyname for '$host'");

  socket($gexSocket, PF_INET, SOCK_STREAM, 0) or logmsg('F', "Couldn't open TCP socket");
  connect($gexSocket,$gexPaddr) or logmsg('F',"Failed to connect");
  print "Opened\n"    if $gexDebug & 16;
}
...
sub sendData
{
  ...
  {
    $valSentFlag=1;
    sendDataPacket($name, $units, $group, $title, $value);
...
sub sendDataPacket
{
  ...
  $packet{'version'} = '1.1';
  $packet{'host'} = $myHost;
  $packet{'short_message'} = "$group $name $value $units";
  #$packet{'full_message'} = 'optional long message';
  $packet{'timestamp'} = $intSeconds + $intUsecs/1000000.0;
  $packet{'level'} = 6;
  $packet{'_collectl_name'} = $name;
  $packet{'_collectl_units'} = $units;
  $packet{'_collectl_group'} = $group;
  $packet{'_collectl_title'} = $title;
  $packet{'_collectl_value'} = $value;

  my $string = encode_json \%packet;

  print $string if $gexDebug & 4;
...
usage: --export=gelf,host:port[,options]
